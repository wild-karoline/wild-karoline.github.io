...
Funkce se v C skládá z tzv. function head, který tvoří return type neboli návratový typ, identifier neboli jméno a parameter(list) neboli vstupy, které jsou však nepovinné. Následuje body, blok proveditelného kódu.
...
Funkce, která má něco vracet, má návratový datový typ jinou než void, musí mít na konci klíčové slovo *return* následované tím, co má funkce vracet (tzv. return value). Toto klíčové slovo ukončuje funkci a vrací nějakou hodnotu na volajícího. Klíčové slovo *return* se může nacházet na vícero místech ve funkci. V případe main funkce jsme zatím vraceli pouze hodnotu 0 - 0 značí úspěch, jakákoliv jiná hodnota chybu.
